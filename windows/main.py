...
class SinglishInputController(QObject):
    ...
    def handle_replace_text(self, delete_count, selected_word, extra_char):
        ...
        Uses clipboard copy-paste (via Ctrl+V) to bypass keyboard layout mapping issues,
        ...
        for _ in range(delete_count):
            ...
            time.sleep(0.005) # Brief pause to let the text clear on Windows
        ...
        try:
            ...
            # Send Ctrl+V to paste (universally supported in Windows applications)
            with self.keyboard_controller.pressed(Key.ctrl):
                self.keyboard_controller.press('v')
                self.keyboard_controller.release('v')
                
            # Wait 100ms for the target application to process the paste event.
            # We use a local QEventLoop so the GUI event loop remains responsive.
            loop = QEventLoop()
